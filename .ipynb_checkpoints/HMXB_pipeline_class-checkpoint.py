...
class pipeline:
    ...
    def solar_proper_motion(self,table):
        ...
        Kr_mul_sol = self.U_sun*np.sin(long_rad) - self.V_sun*np.cos(lat_rad)
        table['mu_l_sol'] = (Kr_mul_sol/(self.k*dist)) # need to specifiy units to subtract from proper motion
    ...
    def peculiar_velocity(self, table):
        ...
        V_pec.units = u.km/u.s
        table['Peculiar Velocity'] = V_pec
        return table
    def lay_pipe(self,table,filename,filetype):
        '''Combine everything'''
        ...
        return table
test_table = "Test_XRBCATS.csv"
if __name__ == "__main__":
    test_massive = pipeline(test_table,fmt='csv')
    my_table = test_massive.make_table()
    x = test_massive.lay_pipe(my_table, filename='test_lay_pipe.ecsv',filetype='ascii.ecsv')